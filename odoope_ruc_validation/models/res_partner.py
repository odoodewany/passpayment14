...
class ResPartner(models.Model):
    ...
    company_id = fields.Many2one(
        'res.company', 'Company', required=False, index=True, default=_default_company)
    country_id = fields.Many2one(default=_default_country)
    commercial_name = fields.Char(string="Commercial Name")
    state = fields.Selection(
        [('habido', 'Habido'), ('nhabido', 'No Habido')], string="State")
    ...
    def get_data_ruc(self):
        ...
        if result:
            ...
            self.commercial_name = str(
                result['commercial_name'] or result['business_name']).strip()
    ...
    def sunat_connection(self, ruc):
        ...
        try:
            url_numRnd = session.get(
                'https://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/jcrS00Alias?accion=consPorRazonSoc&razSoc=BVA%20FOODS', headers=headers, timeout=12).content
            ...
            data_ruc = {'accion': 'consPorRuc', 'nroRuc': ruc,
                        'numRnd': numRnd, 'actReturn': '1', 'modo': '1'}
            html_doc = session.post(url=url_sunat, data=data_ruc,
                                    headers=headers, timeout=(15, 20))
            html_info = BeautifulSoup(html_doc.content, 'html.parser')
            div_info = html_info.find_all("div", {"class": "list-group"})
            div_p_info = div_info[0].find_all(
                "p", {"class": "list-group-item-text"})
            div_h4_info = div_info[0].find_all(
            ...
            data['type_of_taxpayer'] = (
                div_p_info[sunat_cons.type_of_taxpayer.value].contents[0])
            data['estado'] = (
                div_p_info[sunat_cons.taxpayer_state.value].contents[0])
            data['contributing_condition'] = (div_p_info[sunat_cons.contributing_condition.value].contents[0]).replace('\r', '') \
                .replace('\n', '').strip()
            data['commercial_name'] = (
                div_p_info[sunat_cons.commercial_name.value].contents[0]).replace('-', '').strip()

            residence = (
                div_p_info[sunat_cons.tax_residence.value].contents[0])
    ...
    def sunat_connection_multi(self, ruc):
        ...
        if self.env.company.l10n_pe_use_proxy:
            url_proxy = "http://%s:%s" % (self.env.company.l10n_pe_proxy_ip,
                                          self.env.company.l10n_pe_proxy_port)
            session.proxies = {
                "http": url_proxy,
                "https": url_proxy,
            }
        ...
        try:
            ...
            text_captcha = session.post(
                url=captcha, data={'accion': 'random'}, headers=headers)
            data_ruc = {'accion': 'consManual',
                        'selRuc': ruc, 'numRnd': text_captcha}
            html_doc = session.post(url=url_sunat, data=data_ruc,
                                    headers=headers, timeout=(15, 20))
            html_info = BeautifulSoup(html_doc.content, 'html.parser')
            ...
            dist_short_id = self.env['l10n_pe.res.city.district'].search(
                [('name', '=', json_datos['distrito'])], limit=1)
            if dist_id:
                l10n_pe_district = dist_id
            else:
                l10n_pe_district = dist_short_id
    ...
    def reniec_connection(self, dni):
        ...
        data = {}
        try:
            response = session.get(url=url_reniec.format(
                dni=dni), verify=False, headers=headers).text
            values_response = response.replace('&Ntilde;', 'Ñ')
            result = json.loads(values_response)
            data['nombre'] = (result['nombres'] + " " +
                              result['apellido_paterno'] + " " + result['apellido_materno'])
        except Exception:
            self.alert_warning_vat = True
            data = False
        return data
    ...
    def jne_connection(self, dni):
        ...
        data = {}
        try:
            response = session.post(url=url_reniec, json=dni_value,
    ...
    def free_api_connection(self, dni):
        url = 'https://dni.optimizeperu.com/api/prod/persons/{dni}'.format(
            dni=dni)
        headers = {
        ...
        try:
            ...
            name = result.get('first_name') + " " + \
                result.get('last_name') + " " + result.get('name')
            data['nombre'] = name
        except Exception:
            self.alert_warning_vat = True
            data = False
