	...
	def compute_request_data(self):
		for req in self:
			...
			if req.log_ids:
				...
				log_id_ose_accepted = req.log_ids.filtered(lambda r: r.sunat_accepted).sorted('date', reverse = True)
	...
	def action_api_connect_sfs(self, title):
		...
		if os.path.exists(path_file_rpta):
			...
			for filename in archive.namelist():
				if 'xml' in filename:
					filexml = open(path_file_rpta, 'rb')
					out = filexml.read()
					filexml.close()
					# rec.output_xml = base64.b64encode(out)
					# rec.xml_name = path_file_rpta
					fxml = archive.open(filename)
					tree = ET.parse(fxml)
					root = tree.getroot()
					descTag = ET.SubElement(root, "cbc:Description")
					indTag = root.getchildren().index(descTag)
					if indTag > 12:
						obs = root[indTag - 1][0][2].text
						for i in range(9, indTag - 3):
							obs + root[i].text
					else:
						obs = root[indTag - 1][0][2].text
					# rec.output_digest_value = root[0][0][0][0][0][2][2].text
					# rec.url = 'file:///' + path_file_rpta
					# rec.qrcode = "%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|" % (rec.company_id.vat, 
					#     rec.type_document_id.code, rec.serie_id.name, rec.numero, rec.amount_tax, rec.amount_total,
					#     rec.date_invoice, rec.partner_id.catalog_06_id.code, rec.partner_id.vat, rec.output_digest_value)
					values_log = {
						'request_id': self.id,
						# 'json_sent': json.dumps(json_invoice, indent=4, sort_keys=True),
						# 'json_response': json.dumps(response, indent=4, sort_keys=True),
						'link_document': '',
						'link_cdr': 'file:///' + path_file_rpta,
						'link_pdf': '',
						'link_xml': '',
						'operation_type': '',
						'ose_accepted': False,
						'response': '',            
						'sunat_accepted': True if 'aceptad' in obs else False, 
						'sunat_description': obs,                       
						'sunat_note': '',
						'sunat_responsecode': '',
						'sunat_soap_error': ''}
					#~ Register log
