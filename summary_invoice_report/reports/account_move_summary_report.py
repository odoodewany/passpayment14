...
class SummaryAccountMoveLineReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write('F4', 'Nro de documento del cliente',
                    format21_left_bold)
        sheet.write(
            'G4', 'Razón social del documento del cliente', format21_left_bold)
        sheet.write('H4', 'Condición pago', format21_left_bold)
        sheet.write('I4', 'Moneda', format21_left_bold)
        sheet.write('J4', 'Código referencia', format21_left_bold)
        sheet.write('K4', 'Producto/Servicio', format21_left_bold)
        sheet.write('L4', 'Cantidad', format21_left_bold)
        sheet.write('M4', 'Precio unitario', format21_left_bold)
        sheet.write('N4', 'Subtotal', format21_left_bold)
        sheet.write('O4', 'IGV', format21_left_bold)
        sheet.write('P4', 'Total', format21_left_bold)
        ...
        for summ in lines:
            for row, line in enumerate(summ.move_line_ids, 4):
                ...
                untaxed_amount = taxed_amount = 0
                if not line.exclude_from_invoice_tab:
                    untaxed_amount = line.amount_currency
                elif line.tax_line_id:
                    taxed_amount = line.amount_currency
                ...
                sheet.write(row, 4, document_type,
                            format21_left)
                sheet.write(row, 5, vat,
                            format21_left)
                sheet.write(row, 6, partner,
                            format21_left)
                sheet.write(row, 7, line.move_id.invoice_payment_term_id.name,
                            format21_left)
                sheet.write(row, 8, line.move_id.currency_id.name,
                            format21_left)
                sheet.write(row, 9, line.product_id.default_code,
                            format21_left)
                sheet.write(row, 10, line.product_id.name, format21_left)
                sheet.write(row, 11, line.quantity, format21_left)
                sheet.write(row, 12, line.price_unit, format21_left)
                sheet.write(row, 13, line.price_subtotal, format21_left)
                sheet.write(row, 14, line.price_total - line.price_subtotal, format21_left)
                sheet.write(row, 15, line.price_total, format21_left)
